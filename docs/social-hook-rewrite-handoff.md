...
| `socialHook` required for all future facts (prompts + `export:facts` gate) | ✅ Complete |

**This whole effort is now done end to end** — backfill, sync, nib-social integration, and making it a required part of ongoing authoring. See [Related — resolved this session](#related--resolved-this-session) below for exact detail on all four. Only [Still open](#still-open) below remains, and it's explicitly out of scope for this doc (a pre-existing, unrelated issue this work happened to surface).
...
- **nib-social now consumes `socialHook` — done, opt-in by design.** `src/data/types.ts`'s `Fact` gained `socialHook?: string`. First pass had `App.tsx`'s fact-reset effect auto-default to `fact.socialHook ?? fact.headline` — the user then asked to flip that: Cover hook now defaults to `fact.headline` as it always did, with a **"Use hook" button** (shown only when `fact.socialHook` exists, next to the existing "Reset to app headline" button) to opt into the hand-written hook per post. Reasoning: a fact's `socialHook` was written once, in isolation — it may not fit the format/angle actually chosen for a given post, so swapping it in automatically was the wrong default; it's a deliberate per-post choice now, same UX pattern as the reset button. While in there, also fixed a real gap: `buildThreadsPost` (`src/lib/caption.ts`) never took a hook override at all and was hardcoded to `fact.headline` even when the Cover-hook field was edited — it now takes an optional `hook` param with the same blank-falls-back-to-headline behavior as `buildCaption`, so it follows whatever the Cover hook field currently holds (headline by default, the hook if "Use hook" was clicked). `buildPostGuide` now takes the already-resolved `threadsPost` from `App.tsx` instead of recomputing it internally, so the exported guide can't drift from what's shown in the studio. New tests added to `caption.test.ts` for the `buildThreadsPost` hook path; full suite green except one **pre-existing, unrelated** failure in `dailyFact.test.ts` (stale pinned date→factId pairs, confirmed by testing against the pre-sync `facts.json` — same 4 failures either way, not caused by this change).
- **`socialHook` is now required for every future fact — done.** `scripts/export-facts.ts` hard-fails (throws, lists the offending fact ids) if any CSV row's `socialHook` cell is blank — verified by deliberately blanking one, confirming the throw, then restoring. `prompts/draft-facts.md`'s schema now generates `socialHook` alongside `headline`/`body`/`summary` at drafting time (with the same 10-formula/no-fabrication guidance inlined, not just cross-referenced). `prompts/validate-fact.md` now explicitly checks `socialHook` for unsupported claims. `prompts/rewrite-fact.md` now carries `socialHook` through instead of silently dropping it during the optional polish pass. Also fixed `docs/fact-writing-and-quality-guide.md` §8, whose CSV column list was stale and missing `themes`/`socialHook` entirely (pre-existing staleness, unrelated to this session, fixed while touching the same doc). This is no longer a one-time backfill — it's now a standing requirement enforced the same way `headline`/`body`/`summary` already are.
...
- **No in-studio auto-suggestion for `socialHook`.** Drafting still happens the same way `headline`/`body`/`summary` do — an AI assistant following `prompts/draft-facts.md`, human-reviewed — not a one-click "generate a hook" button inside `nib-social` itself. That's an intentional, still-standing choice (see growth-pipeline-handoff.md's "Gap: hook rewriting is still manual" section for the alternatives considered and rejected), not an oversight.
