...
```json
{
  ...
  "socialHook": "On Venus, next year would arrive before tomorrow does."
}
```
...
| `socialHook`      | `String`   | ✅       | **Instagram-only.** A rewrite of `headline` for the nib-social carousel/caption that opens a curiosity gap instead of closing one — `headline` can safely state the fact outright because the app reader already opened the card; a cold Instagram scroll needs the payoff withheld until the swipe. Added 2026-08-08; the full 966-fact backfill completed 2026-08-11, and **`export:facts` now hard-fails if any fact's CSV cell is blank** — write one for every new fact at drafting time (see `prompts/draft-facts.md`, and the method notes in `docs/social-hook-rewrite-handoff.md`: read the full fact, pick a formula from nib-social's growth-strategy doc §9, never introduce a claim not already in that fact's own `body`/`summary`). Never read or displayed by the Nib app itself (unknown keys are silently ignored by `Codable`); consumed only by `nib-social`. |
