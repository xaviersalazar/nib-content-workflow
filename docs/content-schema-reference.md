...
| `socialHook`      | `String?`  | ❌       | **Instagram-only.** A rewrite of `headline` for the nib-social carousel/caption that opens a curiosity gap instead of closing one — `headline` can safely state the fact outright because the app reader already opened the card; a cold Instagram scroll needs the payoff withheld until the swipe. Added 2026-08-08, backfilled gradually — most facts don't have one yet. **Omitted, not an empty string, when unset** — `export:facts` only writes the key when the CSV cell is non-blank. Never read or displayed by the Nib app itself (unknown keys are silently ignored by `Codable`); consumed only by `nib-social`. |
